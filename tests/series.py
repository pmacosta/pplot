...
# pylint: disable=F0401,R0201,R0204,R0205,R0903,R0913,W0201,W0212,W0232,W0621
...
sys.path.append("..")
from tests.gen_ref_images import DPI, FHEIGHT, FWIDTH, create_marker_line_type_image
...
class TestSeries(object):
    ...
    @pytest.mark.parametrize(
        "color",
        [None, "moccasin", 0.5, "#ABCDEF", (0.5, 0.5, 0.5), [0.25, 0.25, 0.25, 0.25]],
    )
    def test_color(self, default_source, color):
        ...
        obj = pplot.Series(data_source=default_source, label="test", color=color)
    ...
    def test_data_source(self, default_source):  # noqa: D202
        ...
        obj = TestSource()
        obj.indep_var = np.array([5, 6, 7, 8])
        obj.dep_var = np.array([0, -10, 5, 4])
        ...
        assert (obj.indep_var == np.array([5, 6, 7, 8])).all()
        assert (obj.dep_var == np.array([0, -10, 5, 4])).all()

    @pytest.mark.series
    def test_data_source_exceptions(self):  # noqa: D202
        ...
        obj = pplot.BasicSource(
            indep_var=np.array([1, 2, 3, 4]), dep_var=np.array([10, 20, 30, 40])
        )
        obj._indep_var = None
    ...
    def test_interp(self, default_source):
        ...
        source_obj = pplot.BasicSource(indep_var=np.array([5]), dep_var=np.array([0]))
        items = [
            (None, default_source),
            ("STRAIGHT", source_obj),
            ("STEP", source_obj),
        ...
        for item in items:
            obj = pplot.Series(data_source=default_source, label="test", interp=item)
    ...
    def test_line_style(self, default_source):
        ...
        pplot.Series(data_source=default_source, label="test", line_style=None)
        items = ["-", "--", "-.", ":"]
        for item in items:
            obj = pplot.Series(
    ...
    def test_marker(self, default_source):
        ...
        obj = pplot.Series(data_source=default_source, label="test", marker=None)
        assert obj.marker is None
        obj = pplot.Series(data_source=default_source, label="test", marker="D")
        assert obj.marker == "D"
        obj = pplot.Series(data_source=default_source, label="test")
        assert obj.marker == "o"
    ...
    def test_secondary_axis(self, default_source):
        ...
        pplot.Series(data_source=default_source, label="test", secondary_axis=None)
        items = [False, True]
        for item in items:
            obj = pplot.Series(
    ...
    def test_scale_indep_var(self, default_source):
        ...
        obj = pplot.Series(data_source=default_source, label="test", interp=None)
        ...
        indep_var = np.array([2.5, 3.0, 3.5, 4.0])
        dep_var = np.array([0.0, -2.5, 1.25, 1.0])
    ...
    @pytest.mark.parametrize("interp", ["STRAIGHT", "STEP", "CUBIC", "LINREG"])
    @pytest.mark.parametrize("line_style", [None, "-", "--", "-.", ":"])
    def test_images(self, marker, interp, line_style, tmpdir, ref_source):
        ...
        create_marker_line_type_image(
            mode, test_dir, ref_source, marker, interp, line_style, olist, False
        )
